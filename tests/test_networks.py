...
def test_config_with_basics():
    ...
    config = generate_dnsmasq_config(spec, "/tmp")
    assert "pid-file=/tmp/pidfile" in config
...
def test_config_with_logging():
    ...
    assert "log-facility=/tmp/dnsmasq.log" in config
...
def test_config_with_dhcp():
    ...
    assert "dhcp-hostsfile=/tmp/hostsfile" in config
    assert "dhcp-leasefile=/tmp/leases" in config
